...
import { useLabs } from '../../context/serviceIndex.jsx';

function LabsSection({ showToast }) {
  const { messages = [], loading, sendQuestion } = useLabs();
  ...
  async function send() {
    ...
    try {
      ...
      showToast?.('err', 'Unable to contact the AI service.');
    }
  }
  ...
  const ChatMsgs = ({ refEl, cls }) => (
    <div className={cls || 'ai-chat'} ref={refEl}>
      ...
      {loading && <div className="ai-msg"><div className="av av-sm av-g" style={{ fontSize: 10 }}>AI</div><div className="ai-typing"><div className="ai-dot"/><div className="ai-dot"/><div className="ai-dot"/></div></div>}
    </div>
  );
