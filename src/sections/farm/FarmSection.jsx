...
function FarmSection({walletIST,walletNaira,onSpend,showToast,listOpen,setListOpen,activeFarmTab,setActiveFarmTab}){
  ...
  const [farms,setFarms]=useState(SEED.farms);
  const [produce]=useState(SEED.produce);
  const [equipment]=useState(SEED.equipment);
  const [supplies]=useState(SEED.supplies);
  const [labor]=useState(SEED.labor);
