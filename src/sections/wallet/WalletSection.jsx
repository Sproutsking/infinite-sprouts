...
import { useWallet } from '../../context/serviceIndex.jsx';

function WalletSection({ showToast }) {
  const { wallet = {}, transactions = [], addTransaction, adjustBalance } = useWallet();
  ...
  async function doSend() {
    ...
    if (!sendPhone || !amount || amount <= 0) return;
    if (amount > walletNaira) {
      showToast('err', 'Insufficient Naira balance.');
      return;
    }
    try {
      await adjustBalance({ naira_balance: Math.max(0, walletNaira - amount) });
      await addTransaction({ type: 'out', title: 'OPay transfer', sub: `Sent to ${sendPhone}`, amount, wallet: 'naira' });
      setSendOpen(false);
      setSendPhone('');
      setSendAmt('');
      showToast('ok', '₦' + fmt(amount) + ' sent via OPay!');
    } catch (error) {
      console.error('Send error', error);
      showToast('err', 'Unable to complete the transfer.');
    }
  }
  ...
  async function doTopup() {
    ...
    try {
      await adjustBalance({ naira_balance: walletNaira + amount });
      await addTransaction({ type: 'in', title: 'Wallet top-up', sub: `${topupMethod.toUpperCase()} top-up`, amount, wallet: 'naira' });
      setTopupOpen(false);
      setTopupAmt('');
      showToast('ok', '₦' + fmt(amount) + ' top-up initiated!');
    } catch (error) {
      console.error('Top-up error', error);
      showToast('err', 'Unable to top up your wallet.');
    }
  }

  async function doBuyIST() {
    if (!selPkg) return;
    const istAmount = selPkg.val;
    const cost = istAmount * 8;
    if (cost > walletNaira) {
      showToast('err', 'Insufficient Naira balance to buy IST.');
      return;
    }
    try {
      await adjustBalance({ naira_balance: walletNaira - cost, ist_balance: walletIST + istAmount });
      await addTransaction({ type: 'out', title: 'Bought IST', sub: selPkg.amt, amount: cost, wallet: 'naira' });
      await addTransaction({ type: 'in', title: 'Bought IST', sub: selPkg.amt, amount: istAmount, wallet: 'ist' });
      setBuyISTOpen(false);
      setSelPkg(null);
      showToast('ok', selPkg.amt + ' purchased successfully!');
    } catch (error) {
      console.error('Buy IST error', error);
      showToast('err', 'Unable to purchase IST.');
    }
  }
